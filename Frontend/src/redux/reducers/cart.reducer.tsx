...
import { ADD_TO_CART, UPDATED_CART, DECREMENT_PRODUCT,OUT_PRODUCT} from "../types_redux/consts";
...
case ADD_TO_CART: {
  ...
  const updatedProducts = state.products.map((product) => {
    ...
    if (newProduct) {
      ...
      return {
        ...
        totalPrice: (product.quantity + (newProduct.quantity || 1)) * product.price,
      };
    }
  ...
  const newItems = newProducts
    .filter((newProduct) => !state.products.some((product) => product._id === newProduct._id))
    .map((newProduct) => ({
      ...newProduct,
  ...
  return { ...state, products: finalProducts, totalPrice: updatedTotalPrice };
}
    ...
    const newItems = newProducts.filter(
      (newProduct) => !state.products.some((product) => product._id === newProduct._id)
    );
 ...
 const prodDeleted= state.products.filter(prod=>prod._id!==productToDelete._id)
